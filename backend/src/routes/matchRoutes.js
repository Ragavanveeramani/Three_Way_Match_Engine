...
router.get('/summary/:poNumber', protect, async (req, res) => {
  try {
    ...
    res.json({
      ...
      totalReceived,
      documents: [
        ...(po ? [{ type: 'PO', number: po.poNumber, date: po.poDate }] : []),
        ...grns.map(g => ({ type: 'GRN', number: g.grnNumber, date: g.grnDate })),
        ...invoices.map(i => ({ type: 'Invoice', number: i.invoiceNumber, date: i.invoiceDate }))
      ]
    });
  } catch (err) {
